...
beautiful.init("/home/replay/.config/awesome/themes/default/theme.lua")
...
vicious.register(netwidget, vicious.widgets.net, "[${eth1 down_kb}/${eth1 up_kb}]|")
...
vicious.register(tempwidget, vicious.widgets.hddtemp, "${/dev/sda}] *C|", 19)

-- Thermal
thermalwidget = widget({ type = "textbox" })
vicious.register(thermalwidget, vicious.widgets.thermal, "[$1|", 19, "thermal_zone0")


-- This is used later as the default terminal and editor to run.
-- terminal = "xterm +tb -fg green -bg black"
terminal = "urxvt"
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "term", "web", "mail", "im"}, s, layouts[1])
end
-- }}}
...
myawesomemenu = {
   { "manual", terminal .. " -e man awesome", freedesktop.utils.lookup_icon({ icon = 'text-html' }) },
   { "edit config", editor_cmd .. " " .. awful.util.getdir("config") .. "/rc.lua", freedesktop.utils.lookup_icon({ icon = 'configure' }) },
   { "restart", awesome.restart, freedesktop.utils.lookup_icon({ icon = 'view-refresh' }) },
...
--                                    { "Terminal", terminal }, {"Tab Terminal", terminal .. " -e screen" }
--                                  }
--                        })
